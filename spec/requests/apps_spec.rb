...
RSpec.describe "Apps", type: :request do
  ...
  describe "GET /apps/:id" do
    ...
    it "assigns @recent_scans limited to 50" do
      create_list(:quality_scan, 60, app: app)
  ...
  describe "POST /apps/:id/scan" do
    ...
    it "enqueues scan job" do
      ...
      post "/code_quality/apps/#{app.id}/scan"

      expect(AppScannerJob).to have_received(:perform_later).with(app.id)
    end
    ...
    it "redirects to app show page" do
      ...
      expect(response).to redirect_to("/code_quality/apps/#{app.id}")
    end
    ...
    it "sets flash notice" do
      ...
      expect(flash[:notice]).to include(app.name)
    end
  end
  ...
  describe "POST /apps/discover" do
    ...
    it "discovers Rails apps in ecosystem" do
      ...
      expect(App.find_by(name: "app1")).to be_present
      expect(App.find_by(name: "app2")).to be_present
    end

    it "sets correct app paths" do
      post "/code_quality/apps/discover"

      app1 = App.find_by(name: "app1")
    ...
    it "sets flash notice with app count" do
      ...
      expect(flash[:notice]).to include(App.count.to_s)
    end

    it "does not duplicate existing apps" do
      create(:app, name: "app1", path: app1_path)

      expect {
        post "/code_quality/apps/discover"
      }.to change(App, :count).by(1) # Only app2 is new
    end
    ...
    it "skips non-Rails directories" do
      ...
      expect(App.find_by(name: "app1")).to be_present
      expect(App.find_by(name: "app2")).to be_nil
    end
  end
end
