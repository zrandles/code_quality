    ...
    context 'when app directory exists' do
      ...
      describe 'reek scanning' do
        ...
        before do
          ...
          allow(File).to receive(:exist?).with(/reek.*\.json/).and_return(true)
          allow(File).to receive(:read).with(/reek.*\.json/).and_return(reek_output.to_json)
      ...
      describe 'summary creation' do
        before do
          ...
          create(:quality_scan, :static_analysis, app: app, severity: "high", metric_value: 45.0)
          create(:quality_scan, :static_analysis, app: app, severity: "medium", metric_value: 25.0)
          create(:quality_scan, :static_analysis, app: app, severity: "low", metric_value: 10.0)
        end
